...
def draw_sphere_marker(position, radius, color):
    ...
    marker_id = p.createMultiBody(basePosition=position,
...
def get_angle(old, new) -> float:
    dir = (new - old)[:2]
    if dir[0] == 0 and dir[1] == 0: return new[2]
    dir = dir / np.linalg.norm(dir)
    return math.acos(np.dot(dir,np.array([1,0])))
